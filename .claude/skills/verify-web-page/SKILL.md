...
`<feature>` は PR の主題に対応した短い識別子（例: `transactions-list`, `csv-upload`）。kebab-case を推奨。
...
ブランチ名は今 push したブランチ。マージ後にブランチを削除すると URL は壊れるが、その時点では PR は閉じてレビューも完了しているので実害はない。長期保存が必要な場合は branch 名の代わりに commit SHA を使うと永続化できる。
