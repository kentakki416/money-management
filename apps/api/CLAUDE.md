...
- **Controller層 → インテグレーションテスト**（`apps/api/test/controller/`）: 自前インフラ（MySQL・Redis）は本物を使い、`supertest` で HTTP レイヤーから検証

### Controller integration テストでモックして良いもの／いけないもの

| 種類 | 例 | 扱い | 理由 |
|---|---|---|---|
| 自前インフラ | MySQL / Redis | **本物**（テスト用 DB に接続） | キー名・TTL・型変換・SQL の誤りなど、mock では検出できない不具合を捕捉する |
...
「メソッドが呼ばれた」だけ検証する mock 駆動の assertion は Controller integration テストでは不十分。**MySQL / Redis の最終状態を直接確認する**。
